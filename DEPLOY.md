...
Relay gateway berjalan di **dedicated EC2** (13.212.26.4), terpisah dari main app EC2.
Lihat [`opt/relay/DEPLOY.md`](./opt/relay/DEPLOY.md) untuk panduan lengkap.

### Quick Update Relay

```bash
rsync -avz -e "ssh -i ~/Downloads/EsluceRelay\(1\).pem -o StrictHostKeyChecking=no" \
  /home/rhnbztnl/Downloads/Berguna/Projects/escluse/opt/relay/ \
  ec2-user@13.212.26.4:/home/ec2-user/escluse/opt/relay/ && \
ssh -i ~/Downloads/EsluceRelay\(1\).pem ec2-user@13.212.26.4 \
  "cd /home/ec2-user/escluse && docker compose -f opt/relay/docker-compose.yml up -d --build"
```
