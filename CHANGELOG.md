...
### Fixed
- `backup.delete` from the deployed backend's raw JSON protocol now routes into the canonical backup handler instead of falling through to the generic shell arm (`sh -c "backup.delete"` → `backup.delete: not found`), so retention pruning actually deletes archives on the node.
...
### Fixed
- Restore is only rejected while the server is actually running — a stopped server in the registry can be restored again.
...
### Fixed
- Agent backups on devices without Docker: `backup.start` archives the server's data directory directly with the local `tar` binary instead of `docker exec`, and `backup.restore` extracts into the server folder without `docker stop`/`docker cp`.
...
- **Java not detected on Termux (v0.5.6 regression)** — The v0.5.6 fallback paths (`Path::exists()` on common Termux paths) still failed on Xiaomi HyperOS. Added `sh -c "command -v java"` shell-based fallback.
...
- **Auto-install Java on Termux when no runtime found** — On Termux (Android), if no podman/docker/Java is available at startup, the agent now auto-installs `openjdk-17` via `pkg install -y` in a background task. Once installed, subsequent "Start Server" commands use the Java DirectExecutor path automatically.
