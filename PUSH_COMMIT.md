...
| `gateway/`, `docker-compose.yml` | `esclusehq/escluse-infra` | Infrastructure - Docker, Reverse Proxy |
...
### gateway/ + docker-compose.yml (Infrastructure)

```bash
# Buat folder sementara untuk combine files
mkdir -p /tmp/escluse-infra
cp -r gateway/ /tmp/escluse-infra/
cp docker-compose.yml /tmp/escluse-infra/

cd /tmp/escluse-infra
git init && \
git remote add origin https://github.com/esclusehq/escluse-infra.git && \
git config user.email "dev@esluce.com" && \
git config user.name "Escluse Dev"
...
cat > README.md << 'EOF'
# Escluse Infrastructure

Docker orchestration, reverse proxy configuration, and deployment setup.

## Contents

- docker-compose.yml
- gateway/Caddyfile
- gateway/Caddyfile.prod
- gateway/nginx.landing.conf
EOF

git add .gitignore README.md docker-compose.yml gateway/
git commit -m "Initial commit: Escluse Infrastructure"
...
escluse/
├── agent/
│   ├── solys/           → github.com/esclusehq/solys
│   └── agent-core/     → github.com/esclusehq/agent-core
├── app/                → github.com/esclusehq/escluse-dashboard
├── api/                → github.com/esclusehq/escluse-cloud
│   └── worker/         (included in api)
├── landing-page-escluse/ → github.com/esclusehq/escluse-landing-page
├── packages/           → github.com/esclusehq/escluse-sdk
├── docs/               → github.com/esclusehq/escluse-docs
├── gateway/            → github.com/esclusehq/escluse-infra
├── docker-compose.yml  → github.com/esclusehq/escluse-infra
