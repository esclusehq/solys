...
# Usage: curl -fsSL https://get.esclusehg.com/latest/install.sh | bash
#        curl -fsSL https://get.esclusehg.com/latest/install.sh | bash -s v1.2.3
set -euo pipefail
...
# --- Termux detection ---
detect_termux() {
    if [ -d "/data/data/com.termux/files/usr" ] && [ "$(uname -o 2>/dev/null)" = "Android" ]; then
        IS_TERMUX=true
...
build_urls() {
    if [ "$VERSION" = "canary" ]; then
        BASE_URL="https://get.esclusehg.com/canary"
        info "Using canary build"
    elif [ "$VERSION" = "latest" ]; then
        BASE_URL="https://get.esclusehg.com/latest"
        info "Using latest version"
    else
        # Strip leading 'v' if present
        V="${VERSION#v}"
        BASE_URL="https://get.esclusehg.com/v${V}"
...
_prompt() {
    ...
    if [ "$IS_TERMUX" = true ]; then
        # Termux: /dev/tty may not exist; read directly from stdin
...
[server]
backend_url = "wss://app.esclusehg.com/api/ws/node"
api_key = "${ak_val}"
...
root_check() {
    ...
    if [ "$(id -u)" -ne 0 ]; then
        fail "This installer must be run as root. Please re-run with: sudo bash -c \"\$(curl -fsSL https://get.esclusehg.com/latest/install.sh)\""
    fi
}
